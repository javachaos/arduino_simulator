...
`rust_gui` is the native desktop front end for `arduino_simulator`.
...
- load a board-definition JSON document and render its primary PCB-backed member directly in the GUI
...
1. Open or save a simulation project JSON document that captures the host board, firmware source, and PCB path
...
4. Load a board-definition JSON document and inspect the rendered primary board geometry
